...
def create_remote_maintainer_scripts(package_root: Path, manifest: dict, install_dir: Path) -> None:
    ...
    postinst = "\n".join(
        [
            ...
            '  if aria2c --allow-overwrite=true --auto-file-renaming=false --check-integrity=true --connect-timeout=5 --dir "$install_dir" --follow-metalink=mem ${metalink_protocol_arg:+$metalink_protocol_arg} --max-tries=1 --out "$(basename "$target_file")" --timeout=20 "$metalink_url"; then',
            "    downloaded=1",
            "  fi",
            "fi",
            "",
